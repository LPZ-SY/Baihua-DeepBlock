...
def submit_quafu_partition_job(
    ...
    if token_is_jwt:
        # JWT token belongs to SQC composer path (cookie access_token), not main api_token endpoints.
    ...
    for endpoint in endpoints:
        ...
        try:
            ...
            last_backend = backend
            if _is_auth_error(err) and not token_is_jwt:
                break

    summary = " || ".join(attempts[-3:])
    return QuafuJobResult(
        ok=False,
        message=f"Quafu submission failed on all endpoints: {summary}",
        task_id=None,
