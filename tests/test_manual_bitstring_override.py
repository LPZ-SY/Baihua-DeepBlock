...
def test_manual_bitstring_forces_hybrid_mode():
    ...
    with patch("quantum_route_forge.pipeline.submit_quafu_partition_job", return_value=fake_job):
    ...
    assert result.metadata.used_mode == "quafu_quantum_hybrid"
...
def test_invalid_manual_bitstring_is_ignored():
    ...
    assert result.metadata.used_mode == "quafu_submitted_classical_refine"
